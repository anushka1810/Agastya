...
- **Filter chips** (All / Overdue / Partially Paid / Paid) sit directly below the summary, since narrowing the list is the very next thing she does after seeing the total.
...
Kept: the two summary numbers (stacked, not side-by-side, since 375px doesn't have room for both cards horizontally), and a short list of only the students who are Overdue or Payment-Failed, sorted by balance descending, each with a tap-to-call icon next to the guardian's number.

Dropped entirely on mobile: filter chips, checkboxes/bulk-select, and the full 24-student list. None of these serve a "quick glance in the corridor" moment — bulk actions and filtering are a desk task, not a hallway task.
