...
export function StudentListView() {
  ...
  // Group filtered students (Mobile: Overdue/Payment Failed only, sorted descending)
  const mobileFamilyGroups = useMemo(() => {
    if (!data) return [];
    const mobileStudents = (data.students as Student[])
      .filter(s => s.status === 'OVERDUE' || s.status === 'PAYMENT_FAILED')
      .sort((a, b) => b.balance - a.balance);
    return groupByFamily(mobileStudents);
  }, [data]);
  ...
  return (
    <div className="bg-background text-on-background h-screen flex flex-col md:flex-row overflow-hidden">
      ...
      <div className="flex flex-col flex-grow overflow-hidden relative">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer pt-1 focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2 focus-within:ring-offset-background rounded">
                      <input
                        ...
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            e.preventDefault();
                            const isChecking = !allOverdueSelected;
                            if (isChecking) {
                              const newSelected = new Set(selectedIds);
                              overdueStudentIds.forEach(id => newSelected.add(id));
                              setSelectedIds(Array.from(newSelected));
              ...
              {status === 'loading' ? (
                ...
                <>
                  ...
                  <div className="md:hidden space-y-[8px]">
                    {mobileFamilyGroups.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-16 text-center px-4">
                        <span className="material-symbols-outlined text-[48px] text-primary mb-4">celebration</span>
                        <h3 className="text-[16px] font-bold text-on-surface">No overdue students right now — nice work!</h3>
                      </div>
                    ) : (
                      mobileFamilyGroups.map(group => {
                        if (group.students.length > 1) {
                          return (
                            <FamilyCard
                              key={group.familyId}
        ...
        {activeTab === 'Payments' && (
          <div className="hidden md:block fixed bottom-[64px] md:bottom-0 left-0 w-full p-4 bg-gradient-to-t from-surface via-surface to-transparent pointer-events-none z-20">
            <div className="max-w-[1280px] mx-auto flex justify-center md:justify-end md:pl-[250px]">
              <button
                ...
                className="pointer-events-auto w-full md:w-auto bg-primary text-white font-bold py-3 px-6 rounded-lg shadow-md hover:opacity-90 active:scale-[0.98] transition-all flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-surface"
              >
